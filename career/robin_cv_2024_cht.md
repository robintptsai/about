...
4. 在專案執行階段領導相關的會議與討論, 控制專案時程目標和資源, 驅動與加速內外部成員的執行或提供協助指引
5. 開發與保存專案相關紀錄,工作產出紀錄與交付(系統設計文件, 系統架構圖, 程式碼, 現場拓樸圖, 系統設定, 系統帳密), 以向贊助者呈報專案資產與成果
6. 開發制定系統可用與使用目標(SLA), 維運模式 (人員,實踐)來確認系統健全運作以滿足企業營運, 並以資料事實對利害關係人提供維運能見與洞察
7. 評估系統的安全合規需求與實踐,並提供執行方法, 以促成企業獲得認證(GDPR,CCPA,CIS,SOC,FIPS,FEDRAMP,JP MARK,...)
8. 提議創新方案(自動化,AI生成加速)來達成更成熟的系統維運效率與產值(cost/performance)
